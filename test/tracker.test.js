...
const DATA = path.join(__dirname, "../data/tracker");
...
function writeDay() {
  ...
  const events = [
    // 30 min in Cursor (Agency · Agents, +2) — one merged event
    ...
    // 40 min Higgsfield (Agency · Reels, +2), but 10 min of it is AFK-masked
...
test("computeDay: AFK masking, categories, pulse, focus sessions", () => {
  ...
  assert.equal(cats["Agency · Agents"], 30);
  assert.equal(cats["Social feeds"], 10);
  assert.equal(cats["Agency · Reels"], 30);
